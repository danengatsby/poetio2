...
async function request(url: string, options: RequestInit = {}) {
  ...
  if (!result.ok) throw new Error(data?.error || 'Cererea nu a reușit. Încearcă din nou; textul introdus rămâne în formular.');
  if (!data) throw new Error('Răspunsul nu poate fi citit. Reîncearcă salvarea; poemul nu va fi duplicat.');
  return data;
}

export default function Admin({ initialPoems, initialError, translationReady }: { initialPoems: Poem[]; initialError: boolean; translationReady: boolean }) {
  ...
  function replaceDraft(value: Draft | null) {
    setDraft(value); setBaseline(value); setError(''); setSuccess('');
  ...
  async function refresh() {
    ...
    catch (error) { setError(error instanceof Error ? error.message : 'Lista nu poate fi încărcată.'); }
  ...
  async function save(event: React.FormEvent, originalOnly = false) {
    ...
    try {
      ...
      if (selectedImage) {
        ...
        if (!uploaded.ok || !uploadData?.image?.id) throw new Error(uploadData?.error || 'Imaginea nu a putut fi încărcată. Textul și imaginea aleasă rămân în formular; încearcă din nou.');
    ...
    } catch (error) { setError(error instanceof Error ? error.message : 'Salvarea nu a reușit. Textul introdus rămâne în formular.'); }
    finally { setBusy(false); }
  }
  async function remove() {
    ...
    } catch (error) { setDeleteOpen(false); setError(error instanceof Error ? error.message : 'Ștergerea nu a reușit.'); }
    finally { setBusy(false); }
  }
  ...
  return <div lang="ro" className="admin-page">
    ...
    <main className="admin-main shell">
      ...
      <div className="admin-feedback" aria-live="polite" aria-atomic="true">{success && <p className="success-message">{success}</p>}{error && <div className="error-message" role="alert"><p>{error}</p><button disabled={locked} className="text-button" onClick={() => guard(refresh)}>Reîncarcă lista</button><a className="text-button" href="/signin-with-chatgpt?return_to=%2Fadmin" target="_blank" rel="noopener noreferrer">Autentificare într-o filă nouă</a></div>}</div>
